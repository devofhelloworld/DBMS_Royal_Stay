...
exports.postupdateroom = (req,res,next)=>{
  ...
  const {room_name,price,room_des,status,availability,img} = req.body;
  rooms.updateroomdetails(room_name,price,room_des,status,availability,id,img).then(()=>{
...
exports.getaddadmin = (req,res,next)=>{
  managehotel.gethotelinfo().then(([[data]])=>{
    const hoteldata = data;
    res.render('adminaccess',{isloggedin:req.session.isloggedin,isadminloggedin: req.session.isadminloggedin,editing:'false',hoteldata:hoteldata,admindata:'',useremail: req.session.useremail});
  })
}
...
exports.searchresults = (req,res,next)=>{
  ...
  rooms.searchrooms(guests).then(([roomdata])=>{
    managehotel.gethotelinfo().then(([[data]])=>{
      const hoteldata = data;
      res.render('searchresults',{isloggedin:req.session.isloggedin,useremail:req.session.useremail,hoteldata:hoteldata,roomdata:roomdata,guests:guests});
    })
  })
}